...
public class Lab05UsingFunctionalStyle {
...
//        Files.readAllLines(Paths.get("/Users/prabhu/Documents/cities.csv"));
    ...
    private static void printPopulationOf(String countryArg) {
        long total = lines
                .stream()
                .filter(itemArr -> itemArr[1].equalsIgnoreCase(countryArg))
                .mapToLong(itemArr -> Long.parseLong(itemArr[2]))
                .sum();
...
//        }
    ...
    private static List<String[]> readCitiesFile() throws IOException {
        InputStream is = Lab05UsingFunctionalStyle.class.getResourceAsStream("/cities.csv");
